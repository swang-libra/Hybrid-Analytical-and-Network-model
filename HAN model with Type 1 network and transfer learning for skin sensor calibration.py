...
net.hidden.weight.data = (net1_params_from_bs['hidden.weight'] * (max_pixel_value - min_pixel_value) / (max_FOV_size - min_FOV_size) * (max_contact_force - min_contact_force) / (max_pressure_value - min_pressure_value)).mean(dim=1).unsqueeze(dim=1).repeat(1, 5) * 225 / 5
net.hidden.bias.data = (net1_params_from_bs['hidden.bias'] - (max_pixel_value * min_FOV_size - min_pixel_value * max_FOV_size) / (max_FOV_size - min_FOV_size) * net1_params_from_bs['hidden.weight'].sum(dim=1) - min_pressure_value) * (max_contact_force - min_contact_force) / (max_pressure_value - min_pressure_value) + min_contact_force
